<template lang="html">
  <div class="card shadow">
    <div class="card-header">
      <span class="h4">{{ getKegs[3].name }}</span>
    </div>
    <div class="card-body">
      <keg-graphic :percent="percent" :beerColor="getKegs[3].color"></keg-graphic>
    </div>
  ...
  methods: {
    async changeColor (num) {
      ...
      if (res.data.message === 'Color changed') {
        this.$store.state.kegs[3].color = num
      }
    },
    setFull () {
    ...
    tare () {
      this.$socket.emit('tare', this.kegs[3].id)
    }
  },

  sockets: {
    connect () {
      this.$socket.emit('room', this.kegs[3].id)
    },
    weightUpdate (weight) {
      console.log(`Keg4 weight is: ${weight}`)
      this.weight = weight
    }
  }
}
